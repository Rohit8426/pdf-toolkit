...
import { FiUpload, FiTrash2, FiFileText, FiCheckCircle, FiAlertCircle } from 'react-icons/fi';
...
const ConvertToPDF = () => {
  ...
  const formatOptions = [
    { value: 'word', label: 'Word to PDF', icon: '📝' },
    { value: 'excel', label: 'Excel to PDF', icon: '📊' },
    { value: 'powerpoint', label: 'PPT to PDF', icon: '📑' },
    { value: 'image', label: 'Image to PDF', icon: '🖼️' },
  ];
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    const allowedTypes = acceptedFormats[targetFormat];
    
    if (!allowedTypes.includes(selectedFile.type)) {
      setError(`Please upload a ${targetFormat} file (${allowedTypes.map(t => t.split('/')[1]).join(', ')})`);
      return;
    }
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    accept: acceptedFormats[targetFormat].join(','),
    multiple: false,
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/pdf/convert-to', formData, {
        responseType: 'blob',
      ...
      link.href = url;
      link.setAttribute('download', `${file.name.split('.')[0]}_converted.pdf`);
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="p-6 sm:p-8">
          ...
          >
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Convert to PDF</h1>
            <p className="text-gray-600">Transform your documents into professional PDFs</p>
          </motion.div>
          ...
          <motion.div 
            ...
            className="mb-8"
          >
            <label className="block text-sm font-medium text-gray-700 mb-3">Select Conversion Type</label>
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center space-y-3">
              ...
              <p className="text-xs text-gray-500">
                Supported: {acceptedFormats[targetFormat].map(f => f.split('/')[1]).join(', ')}
              </p>
            </div>
          </motion.div>
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {!file ? 'Upload a file' : 'Convert to PDF'}
                </>
              )}
            </motion.button>
          </motion.div>
